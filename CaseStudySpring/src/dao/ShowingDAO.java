...
public class ShowingDAO {
	...
	public Boolean updateShowing(Integer user_id, Integer property_id, String user_message) throws IOException, SQLException {
		Connection conn = null;
		PreparedStatement stmt = null;
		Integer result = null;           
		...
		try {
			...
			stmt.setInt(2, property_id);
			stmt.setString(3, user_message);  
			stmt.setInt(4, user_id);                    // where clause
			stmt.setInt(5, property_id);                // where clause
			result = stmt.executeUpdate();
